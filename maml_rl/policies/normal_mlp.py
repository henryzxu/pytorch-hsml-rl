...
class NormalMLPPolicy(Policy):
    ...
    def forward(self, input, task=None, params=None, enhanced=False):
        ...
        if len(input.shape) == 2:
            output = torch.t(
                torch.stack([torch.cat([torch.from_numpy(np.array(teo)).clone(), embedding[0]], 0) for teo in input], 1).clone())
        if len(input.shape) == 3:
            output = torch.stack([torch.t(
                torch.stack([torch.cat([torch.from_numpy(np.array(teo)).clone(), embedding[0]], 0) for teo in tei], 1).clone()) for tei in input], 1).permute(1, 0, 2)
